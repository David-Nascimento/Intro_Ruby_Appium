...
Entao('esta opção deve ser marcada') do
  @radio_result = find_element(xpath: "//android.widget.RadioButton[contains(@text, 'Ruby')]")
...
Entao ("esta opção deve estar marcada") do
  @check_result = find_element(xpath: "//android.widget.CheckBox[contains(@text, 'Ruby')]")
...
Entao ("todas essas opções devem estar marcadas") do
  @techs.each do |item|
    check_result = find_element(xpath: "//android.widget.CheckBox[contains(@text, '#{item["techs"]}')]")
    expect(check_result.checked).to eql "true"
  end
end